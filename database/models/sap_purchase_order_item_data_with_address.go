...
func (sapPurchaseOrderItemDataWithAddressL) LoadPurchaseOrderSapPurchaseOrderHeaderDataWithAddress(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSapPurchaseOrderItemDataWithAddress interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeSapPurchaseOrderItemDataWithAddress.(*SapPurchaseOrderItemDataWithAddress)
		if !ok {
			object = new(SapPurchaseOrderItemDataWithAddress)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeSapPurchaseOrderItemDataWithAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeSapPurchaseOrderItemDataWithAddress))
			}
		}
	} else {
		s, ok := maybeSapPurchaseOrderItemDataWithAddress.(*[]*SapPurchaseOrderItemDataWithAddress)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeSapPurchaseOrderItemDataWithAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeSapPurchaseOrderItemDataWithAddress))
			}
		}
	}
...
// Exists checks if the SapPurchaseOrderItemDataWithAddress row exists.
func (o *SapPurchaseOrderItemDataWithAddress) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return SapPurchaseOrderItemDataWithAddressExists(ctx, exec, o.PurchaseOrder, o.PurchaseOrderItem)
}
