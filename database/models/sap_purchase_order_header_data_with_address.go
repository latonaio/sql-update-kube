...
func (sapPurchaseOrderHeaderDataWithAddressL) LoadPurchaseOrderSapPurchaseOrderItemDataWithAddresses(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSapPurchaseOrderHeaderDataWithAddress interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeSapPurchaseOrderHeaderDataWithAddress.(*SapPurchaseOrderHeaderDataWithAddress)
		if !ok {
			object = new(SapPurchaseOrderHeaderDataWithAddress)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeSapPurchaseOrderHeaderDataWithAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeSapPurchaseOrderHeaderDataWithAddress))
			}
		}
	} else {
		s, ok := maybeSapPurchaseOrderHeaderDataWithAddress.(*[]*SapPurchaseOrderHeaderDataWithAddress)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeSapPurchaseOrderHeaderDataWithAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeSapPurchaseOrderHeaderDataWithAddress))
			}
		}
	}
...
// Exists checks if the SapPurchaseOrderHeaderDataWithAddress row exists.
func (o *SapPurchaseOrderHeaderDataWithAddress) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return SapPurchaseOrderHeaderDataWithAddressExists(ctx, exec, o.PurchaseOrder)
}
