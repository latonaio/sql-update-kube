...
func callProcess(ctx context.Context, db *database.Mysql, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	case "ProductionOrderConfirmationHeader":
		datum := models.DataPlatformProductionOrderConfirmationHeaderDatum{}
		jmap := map[string]interface{}{}
		keys := make([]string, 0, len(jmap))
		json.Unmarshal(str, &jmap)
		for k, v := range jmap {
	...
	case "UsageControlChainUsageControlChain":
		datum := models.DataPlatformUsageControlChainUsageControlChainDatum{}
		jmap := map[string]interface{}{}
		keys := make([]string, 0, len(jmap))
		json.Unmarshal(str, &jmap)
		for k, v := range jmap {
